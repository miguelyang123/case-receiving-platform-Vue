...
<template>
    <div class="w-[85%] mx-auto py-6 mb-[10rem]">
        ...
        <div class="flex">
            <div class=" relative" @mouseover="mapShow=true" @mouseleave="mapShow=false">
                ...
                <div v-if="mapShow" class="mapAndPriceSelect shadow-xl">
                    <div  v-for="(item,index) in map" class=" text-[1rem] font-bold py-3 pl-6 pr-10 cursor-pointer hover:bg-blue-300 hover:text-white" @click="changeMap(item)">
                        <input type="radio" :id="'map'+index" name="thismap" class="mr-3 cursor-pointer" :value="item" :checked=" thisMap === item">
            ...
            <div class=" relative" @mouseover="priceShow=true" @mouseleave="priceShow=false">
                ...
                <div v-if="priceShow" class="mapAndPriceSelect shadow-xl">
                    <div  v-for="(item,index) in price" class="text-[1rem] font-bold py-3 pl-6 pr-10 cursor-pointer hover:bg-blue-300 hover:text-white" @click="changePrice(item)">
                        <input type="radio" :id="'price'+index" name="thisPrice" class="mr-3 cursor-pointer" :value="item" :checked=" thisPrice === item">
        ...
        <RouterLink :to="{name:'TackCaseDetailsPage',params:{thisList:'abc'}}" class=" relative" v-for="index in 5">
            ...
            <div class="flex">
                ...
                <div class="flex items-center ml-4">
                    ...
                    <p class="text-[#7F7F7F] ml-2 font-bold text-lg">台北</p>
                </div>
                <div class="flex items-center ml-4" >
                    <img src="../assets/img/TackCaseImg/date.png" alt="" width="15" class="h-[15px]">
                    <p class="text-[#7F7F7F] ml-2 font-bold text-lg">2023/09/30</p>
                </div>
            </div>
            ...
            <!-- CSS
                    ...
                    white-space: nowrap;
             -->
            <p class=" text-xl font-bold py-6 pl-6 pr-12 whitespace-nowrap truncate">Lorem ipsum dolor sit amet consectetur adipisicing elit. Totam tempora aut soluta dolor nisi recusandae dolorum aperiam, repellat doloremque placeat, sunt dignissimos quasi quos pariatur impedit itaque. Praesentium quod cumque ratione nobis iusto aliquam repudiandae dolor sapiente enim doloremque minima ad officiis placeat aspernatur illum reiciendis modi, itaque quo? Nemo?</p>
        
            <div class=" text-end py-3">
                <RouterLink to="/" class=" text-white rounded-lg bg-[#FF6E6E] py-3 px-6 mr-12 text-2xl font-bold hover:scale-105 active:scale-95">我要接案</RouterLink>
            </div>
        ...
        <div v-if="page!==0" class="flex justify-end items-end">
           ...
           <button type="button" :class="{'pageBtn':true,'bg-[#ffc8d1] text-[#E12D4A]':index === page}"  v-for="(index) in pageNumCheck" @click="page = index">{{ index }}</button>

           <div v-if="page + Math.floor(this.pageNum/2)+1 < allPage" type="button" class="mx-3 text-[1.125rem] font-bold">...</div>
           <button v-if="page + Math.floor(this.pageNum/2) < allPage" type="button" class="pageBtn" @click="page = allPage">{{ allPage }}</button>
...
<style lang="scss" scoped>
    ...
    .filter{
        ...
        border: 2px #454545 solid; border-radius: 0.5rem;
        width: 175px;
        color: #454545;
        font-weight: bold;
        cursor: pointer;
    ...
    .searchKeyWords{
        outline: none;
        border: 2px #454545 solid; border-radius: 0.5rem 0 0 0.5rem;
        padding: 0.5rem 1rem;
        color: #454545;
        font-weight: bolder;
        cursor: pointer;
    ...
    .mapAndPriceSelect{
        ...
        max-height: 200px;
        overflow: auto;
        border: 2px rgb(200, 200, 200) solid; border-radius: 0.75rem;
        z-index: 50;
    ...
    .pageBtn{
        ...
        padding: 0.75rem; margin: 0 0.25rem;
        font-size: 1.125rem;
